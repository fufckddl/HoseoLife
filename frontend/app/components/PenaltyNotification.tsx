...
export default function PenaltyNotification({ visible, onClose }: PenaltyNotificationProps) {
  ...
  const loadPenalties = async () => {
    try {
      ...
      console.error('처벌 정보 로드 실패:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>처벌 알림</Text>
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>처벌 정보를 불러오는 중...</Text>
            </View>
          ) : penalties.length > 0 ? (
            ...
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>현재 활성화된 처벌이 없습니다.</Text>
            </View>
          )}
