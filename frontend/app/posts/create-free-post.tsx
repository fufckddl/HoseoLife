...
export default function CreateFreePostScreen() {
  const router = useRouter();
  const { isSuspended } = useAuth();
  
  // 자유 게시판은 카테고리가 고정
  const [selectedCategory] = useState('자유');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const postData = {
        ...
        category: selectedCategory, // 자유 게시판 고정
        image_urls: []  // 빈 배열로 시작
      };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle}>자유 게시판 글쓰기</Text>
        <View style={{ width: 24 }} />
      </View>
