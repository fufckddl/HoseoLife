...
export default function CreatePromotionPostScreen() {
  const router = useRouter();
  const { isSuspended } = useAuth();
  
  // 홍보 게시판은 카테고리가 고정
  const [selectedCategory] = useState('홍보');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const postData = {
        ...
        category: selectedCategory, // 홍보 게시판 고정
        image_urls: []  // 빈 배열로 시작
      };
      ...
      Alert.alert('성공', '게시글이 작성되었습니다!', [
        {
          text: '확인',
          onPress: () => {
            router.replace('/posts/promotion');
          }
        }
      ]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle}>홍보 게시판 글쓰기</Text>
        <View style={{ width: 24 }} />
      </View>
