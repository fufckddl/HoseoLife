#!/usr/bin/env python3
"""
불연속 교시 강의를 올바르게 분할하여 DB에 저장하는 스크립트
예: "월0,10,11" -> 월0교시 + 월10-11교시 2개 레코드로 분할
"""
...
def fix_discontinuous_periods():
    """불연속 교시 강의를 올바르게 분할"""
    
    print("🔧 불연속 교시 강의 수정 시작")
    print("=" * 50)
    ...
    try:
        ...
        result = db.execute(text("""
            ...
            FROM courses 
            WHERE lecture_time_room REGEXP '.*[월화수목금토일][0-9]+,[0-9]+.*'
            ...
            if len(new_blocks) > 1:
                ...
                for i, block in enumerate(new_blocks):
                    ...
                    if block['day'] is not None:
                        ...
                        print(f"    블록{i+1}: {day_name}{block['start_period']}-{block['end_period']}교시")
